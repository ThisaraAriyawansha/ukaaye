    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-xl-3 col-lg-3 col-md-12 col-12" style="margin-bottom:32px;">
                ...
                {{-- Search --}}
                <div style="background:#161616; border:1px solid #1f1f1f; border-radius:14px; padding:24px 22px; margin-bottom:20px;">
                    ...
                    <form action="{{ route('products') }}" method="GET" style="display:flex; gap:0; border:1.5px solid #2a2a2a; border-radius:8px; overflow:hidden;">
                        @if(request('category'))
                        ...
                        <input
                            ...
                            style="flex:1; background:#111; border:none; outline:none; padding:10px 14px; font-family:'Mulish',sans-serif; font-size:13px; color:#fff; min-width:0;"
                        >
                        <button
                            type="submit"
                ...
                {{-- Categories --}}
                @if($categories->count())
                <div style="background:#161616; border:1px solid #1f1f1f; border-radius:14px; padding:24px 22px;">
                    ...
                    <a
                        href="{{ route('products') }}"
                        style="display:flex; justify-content:space-between; align-items:center; padding:10px 14px; border-radius:8px; margin-bottom:6px; text-decoration:none; font-family:'Mulish',sans-serif; font-size:13px; font-weight:{{ !request('category') ? '800' : '500' }}; color:{{ !request('category') ? '#fff' : '#888' }}; background:{{ !request('category') ? 'var(--primary-color)' : 'transparent' }}; transition:background .2s;"
                    >
                        <span>All Products</span>
                        <span style="font-size:11px; font-weight:700; background:{{ !request('category') ? 'rgba(255,255,255,.2)' : '#222' }}; color:{{ !request('category') ? '#fff' : '#666' }}; padding:2px 8px; border-radius:20px;">{{ str_pad($categories->sum('products_count'), 2, '0', STR_PAD_LEFT) }}</span>
                    </a>
                    ...
                    <a
                        ...
                        style="display:flex; justify-content:space-between; align-items:center; padding:10px 14px; border-radius:8px; margin-bottom:6px; text-decoration:none; font-family:'Mulish',sans-serif; font-size:13px; font-weight:{{ $active ? '800' : '500' }}; color:{{ $active ? '#fff' : '#888' }}; background:{{ $active ? 'var(--primary-color)' : 'transparent' }};"
                    >
                        <span>{{ $cat->name }}</span>
                        <span style="font-size:11px; font-weight:700; background:{{ $active ? 'rgba(255,255,255,.2)' : '#222' }}; color:{{ $active ? '#fff' : '#666' }}; padding:2px 8px; border-radius:20px;">{{ str_pad($cat->products_count, 2, '0', STR_PAD_LEFT) }}</span>
                    </a>
                    @endforeach
                </div>
                @endif
            ...
            <div class="col-xl-9 col-lg-9 col-md-12 col-12">
                ...
                {{-- Success Alert --}}
                @if(session('success'))
                    <div style="padding:12px 18px; background:#0a2217; border-left:3px solid var(--primary-color); border-radius:8px; font-family:'Mulish',sans-serif; font-size:14px; color:var(--primary-color); margin-bottom:24px;">
                        {{ session('success') }}
                    </div>
                @endif
                ...
                <div class="row g-4">
                    ...
                    <div class="col-xl-4 col-lg-4 col-md-6 col-sm-6 col-12">
                        ...
                        <div
                            class="gwl-pcard"
                            style="background:#161616; border:1px solid #1f1f1f; border-radius:16px; overflow:hidden; display:flex; flex-direction:column; height:100%; transition:border-color .25s, transform .25s;"
                        >
                            {{-- Image --}}
                            <div style="position:relative; height:220px; overflow:hidden; background:#111; flex-shrink:0;">
                            ...
                            <div style="padding:18px 20px 20px; display:flex; flex-direction:column; flex:1; gap:8px;">
                                ...
                                {{-- Title --}}
                                <h5 style="font-family:'Mulish',sans-serif; font-size:15px; font-weight:700; color:#fff; margin:0; line-height:1.4;">
                                    <a href="{{ url($product->slug) }}" style="color:#fff; text-decoration:none;">{{ $product->title }}</a>
                                </h5>
                                ...
                                <div style="display:flex; align-items:center; gap:10px; flex-wrap:wrap;">
                                    @if($product->discounted_price && $product->discounted_price < $product->retail_price)
                                        <span style="font-family:'Mulish',sans-serif; font-size:17px; font-weight:900; color:#fff;">LKR {{ number_format($product->discounted_price, 2) }}</span>
                                        <span style="font-family:'Mulish',sans-serif; font-size:13px; font-weight:500; color:#555; text-decoration:line-through;">LKR {{ number_format($product->retail_price, 2) }}</span>
                                    @else
                                        <span style="font-family:'Mulish',sans-serif; font-size:17px; font-weight:900; color:#fff;">LKR {{ number_format($product->retail_price, 2) }}</span>
                                    @endif
                                </div>
                ...
                @else
                    ...
                    <div style="text-align:center; padding:80px 0;">
                        <div style="width:70px; height:70px; background:#161616; border:1.5px solid #222; border-radius:50%; display:flex; align-items:center; justify-content:center; margin:0 auto 20px;">
                        ...
                        <p style="font-family:'Mulish',sans-serif; font-size:17px; font-weight:700; color:#555; margin:0 0 20px;">No products found.</p>
...
@if($products->hasPages())
                    <div style="display:flex; justify-content:center; align-items:center; gap:8px; margin-top:48px; flex-wrap:wrap;">
                        ...
                        @if($products->onFirstPage())
                            <span style="width:38px; height:38px; display:flex; align-items:center; justify-content:center; border-radius:8px; background:#161616; border:1px solid #333; cursor:not-allowed;">
                                <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M6.5 1.5L3 5L6.5 8.5" stroke="#555" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
                            </span>
                        @else
                            <a href="{{ $products->previousPageUrl() }}" style="width:38px; height:38px; display:flex; align-items:center; justify-content:center; border-radius:8px; background:#161616; border:1px solid var(--primary-color); text-decoration:none;">
                        ...
                        @foreach($products->getUrlRange(1, $products->lastPage()) as $page => $url)
                            ...
                            @else
                                <a href="{{ $url }}" style="width:38px; height:38px; display:flex; align-items:center; justify-content:center; border-radius:8px; background:#161616; border:1px solid #2a2a2a; color:#cccccc; font-family:'Mulish',sans-serif; font-size:13px; font-weight:600; text-decoration:none;">{{ $page }}</a>
                            @endif
                        @endforeach

                        {{-- Next --}}
                        @if($products->hasMorePages())
                            <a href="{{ $products->nextPageUrl() }}" style="width:38px; height:38px; display:flex; align-items:center; justify-content:center; border-radius:8px; background:#161616; border:1px solid var(--primary-color); text-decoration:none;">
                        ...
                        @else
                            <span style="width:38px; height:38px; display:flex; align-items:center; justify-content:center; border-radius:8px; background:#161616; border:1px solid #333; cursor:not-allowed;">
                                <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M3.5 1.5L7 5L3.5 8.5" stroke="#555" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
                            </span>
                        @endif
