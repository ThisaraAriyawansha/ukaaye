<section class="tf-section tf-faq">
    <div class="container">
        <div class="row">
            <div class="col-12">
                <div class="title-heading st-2">
                    <div class="sub-heading clr-pri-3 f-mulish">
                        <svg  xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="77" height="30" viewBox="0 0 77 30">
                            <g>
                              <image width="77" height="30" xlink:href="data:img/png;base64,"/>
                            </g>
                        </svg>
                        <span class="inner-sub st-1">Help & Support</span>
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="77" height="30" viewBox="0 0 77 30">
                            <g>
                              <image width="77" height="30" xlink:href="data:img/png;base64,"/>
                            </g>
                        </svg>
                    </div>
                    <h2 class="title clr-pri-2">We're Here To Help You And Your Little One</h2>
                </div>
            </div>
            <div class="col-md-4">
                <div class="sc-faq wow fadeIn animated" data-wow-delay="0.3ms" data-wow-duration="1200ms">
                    <div class="image">
                        <img src="assets/images/contact/suport-info.png" alt="Customer Support">
                    </div>
                    <div class="content">
                        <h4>Customer Support</h4>
                        <p class="desc">
                            Questions about orders, products, or feeding? We're here Monday-Friday 9am-6pm.
                        </p>
                        <a href="{{ route('contactus') }}" class="fl-btn st-5">
                            <span class="inner">Contact Us</span>
                        </a>
                    </div>
                </div> 
            </div>
            <div class="col-md-4">
                <div class="sc-faq active wow fadeIn animated" data-wow-delay="0.3ms" data-wow-duration="1200ms">
                    <div class="image">
                        <img src="assets/images/contact/new-update.png" alt="Parenting Tips">
                    </div>
                    <div class="content">
                        <h4>Parenting Tips</h4>
                        <p class="desc">
                            Expert advice on baby nutrition, feeding schedules, and introducing solids.
                        </p>
                        <a href="{{ route('blog') }}" class="fl-btn st-5">
                            <span class="inner">Read Blog</span>
                        </a>
                    </div>
                </div> 
            </div>
            <div class="col-md-4">
                <div class="sc-faq wow fadeIn animated" data-wow-delay="0.3ms" data-wow-duration="1200ms">
                    <div class="image">
                        <img src="assets/images/contact/event-program.png" alt="FAQ">
                    </div>
                    <div class="content">
                        <h4>FAQs</h4>
                        <p class="desc">
                            Shipping, returns, product info, and everything you need to know.
                        </p>
                        <a href="{{ route('faq') }}" class="fl-btn st-5">
                            <span class="inner">View FAQs</span>
                        </a>
                    </div>
                </div> 
            </div>
        </div>  
    </div>
</section>