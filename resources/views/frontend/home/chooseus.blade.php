<section class="tf-section tf-sc-about1">
    <div class="container">
        <div class="row">
            <div class="col-xl-6 col-lg-8 col-md-12 col-12">
                <div class="sc-about-1 wow fadeInLeft animated" data-wow-delay="0.3ms" data-wow-duration="1400ms">
                    <div class="title-heading">
                        <div class="sub-heading clr-pri-3 f-mulish"><span class="inner-sub st-2">Why Choose Us</span>
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="77" height="30" viewBox="0 0 77 30">
                                <g>
                                <image width="77" height="30" xlink:href="data:img/png;base64,"/>
                                </g>
                            </svg>
                        </div>
                        <h2 class="title clr-pri-2">Pure, Safe Nutrition 
                            From Day One
                        </h2>
                    </div>
                    <div class="inner">
                        <p class="wrap f-mulish clr-pri-4">Parent-founded Sri Lankan brand. No added sugar, salt, or preservatives. Just pure, organic goodness for your little one.</p>
                        <ul>
                            <li class="st-1 fx"><i class="fas fa-check"></i><p class="clr-pri-2">100% Natural & Organic</p></li>
                            <li class="st-2 fx"><i class="fas fa-check"></i><p class="clr-pri-2">No Preservatives</p></li>
                            <li class="st-3 fx"><i class="fas fa-check"></i><p class="clr-pri-2">Pediatrician Approved</p></li>
                            <li class="st-4 fx"><i class="fas fa-check"></i><p class="clr-pri-2">Affordable Prices</p></li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>