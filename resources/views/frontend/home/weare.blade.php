<section class="tf-section-top tf-discovery">
    <div class="container">
        <div class="row">
            <div class="col-12">
                <div class="title-heading st-2">
                    <div class="sub-heading clr-pri-3 f-mulish">
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="77" height="30" viewBox="0 0 77 30">
                            <g>
                                <image width="77" height="30" xlink:href="data:img/png;base64,"/>
                            </g>
                        </svg>
                        <span class="inner-sub st-1">Essence of Nature</span>
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="77" height="30" viewBox="0 0 77 30">
                            <g>
                                <image width="77" height="30" xlink:href="data:img/png;base64,"/>
                            </g>
                        </svg>
                    </div>
                    <h2 class="title clr-pri-2">Discover Why Sri Lankan Parents Choose Aromat</h2>
                </div>
            </div>
            <div class="col-12">
                <div class="fl-discovery">
                    <div class="sc-discovery wow fadeIn animated" data-wow-delay="0.3ms" data-wow-duration="800ms">
                        <div class="inner-discovery">
                            <div class="box-icon st-1">
                                <i class="fas fa-leaf"></i>
                            </div>
                            <h5 class="title"><a href="{{ route('about') }}">Credibility</a></h5>
                            <p class="sub f-mulish">Certified organic, no additives. Every batch safety-tested.</p>
                            <a href="{{ route('about') }}" class="fl-btn st-10">
                                <span class="inner">read more</span>
                            </a>
                        </div>
                    </div><!--/.sc-discovery-->

                    <div class="sc-discovery active wow fadeIn animated" data-wow-delay="0.3ms" data-wow-duration="1000ms">
                        <div class="inner-discovery">
                            <div class="box-icon st-2">
                                <i class="fas fa-smile"></i>
                            </div>
                            <h5 class="title"><a href="{{ route('about') }}">Love to eat</a></h5>
                            <p class="sub f-mulish">Naturally sweet, smooth textures happy little taste buds love.</p>
                            <a href="{{ route('about') }}" class="fl-btn st-10">
                                <span class="inner">read more</span>
                            </a>
                        </div>
                    </div><!--/.sc-discovery-->

                    <div class="sc-discovery wow fadeIn animated" data-wow-delay="0.3ms" data-wow-duration="1200ms">
                        <div class="inner-discovery">
                            <div class="box-icon st-3">
                                <i class="fas fa-seedling"></i>
                            </div>
                            <h5 class="title"><a href="{{ route('about') }}">Natural & Organic</a></h5>
                            <p class="sub f-mulish">No pesticides, no preservatives. Pure, wholesome goodness.</p>
                            <a href="{{ route('about') }}" class="fl-btn st-10">
                                <span class="inner">read more</span>
                            </a>
                        </div>
                    </div><!--/.sc-discovery-->

                    <div class="sc-discovery wow fadeIn animated" data-wow-delay="0.3ms" data-wow-duration="1400ms">
                        <div class="inner-discovery">
                            <div class="box-icon st-4">
                                <i class="fas fa-clock"></i>
                            </div>
                            <h5 class="title"><a href="{{ route('about') }}">Easy for parents</a></h5>
                            <p class="sub f-mulish">Ready-to-serve. No prep, no mess, just wholesome nutrition.</p>
                            <a href="{{ route('about') }}" class="fl-btn st-10">
                                <span class="inner">read more</span>
                            </a>
                        </div>
                    </div><!--/.sc-discovery-->

                    <div class="sc-discovery wow fadeIn animated" data-wow-delay="0.3ms" data-wow-duration="1600ms">
                        <div class="inner-discovery">
                            <div class="box-icon st-5">
                                <i class="fas fa-globe-asia"></i>
                            </div>
                            <h5 class="title"><a href="{{ route('about') }}">Imported Premium</a></h5>
                            <p class="sub f-mulish">World-class certified nutrition at everyday Sri Lankan prices.</p>
                            <a href="{{ route('products') }}" class="fl-btn st-10">
                                <span class="inner">Shop Now</span>
                            </a>
                        </div>
                    </div><!--/.sc-discovery-->
                </div>
            </div>
        </div>
    </div>
</section>