<section class="tf-section tf-about-full">
    <div class="container">
        <div class="row">
            <div class="col-12">
                <div class="title-heading st-2 text-center">
                    <div class="sub-heading clr-pri-3 f-mulish">
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="77" height="30" viewBox="0 0 77 30">
                            <g>
                              <image width="77" height="30" xlink:href="data:img/png;base64,"/>
                            </g>
                        </svg>
                        <span class="inner-sub st-1">Our Story</span>
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="77" height="30" viewBox="0 0 77 30">
                            <g>
                              <image width="77" height="30" xlink:href="data:img/png;base64,"/>
                            </g>
                        </svg>
                    </div>
                    <h2 class="title clr-pri-2">A Sri Lankan Story <span class="clr-pri-3">Made With Love</span></h2>
                </div>
            </div>
        </div>
        <div class="row align-items-center g-5">
            <div class="col-lg-6 col-md-12">
                <div class="story-img-wrapper wow fadeInLeft animated" data-wow-delay="0.3ms" data-wow-duration="1200ms">
                    <div class="story-img-border-frame">
                        <div class="story-img-inner">
                            <img src="assets/images/about/ryan-fields-Xz7MMD5tZwA-unsplash.jpg" alt="Aromat family in Piliyandala">
                        </div>
                    </div>
                    <div class="story-img-dots"></div>
                </div>
            </div>
            <div class="col-lg-6 col-md-12">
                <div class="story-content wow fadeInRight animated" data-wow-delay="0.3ms" data-wow-duration="1200ms">
                    <p class="f-mulish clr-pri-4">At Aromat, we are proud to be a Sri Lankan company dedicated to nourishing the next generation with rich, nutritious, and affordable baby food. As parents ourselves, we truly understand the importance of providing healthy, safe, and wholesome meals for our little ones. Once we were battling for it and now, we are bringing out a solution for all Sri Lankan parents.</p>
                    <p class="f-mulish clr-pri-4">We also recognize how challenging and expensive it can be in Sri Lanka to find high-quality baby food that parents can trust. That is why we are committed to making premium nutrition accessible to every family.</p>
                    <p class="f-mulish clr-pri-4">Located in Piliyandala, Aromat manufactures and imports a wide range of baby food products designed for babies from 6 months onward and growing children. Our product range includes baby cereals, baby biscuits, puffs, date powder, honey, baby curry mixes, baby drinks, and baby yogurts—carefully crafted to support healthy growth and development.</p>
                    <div class="story-quote-box">
                        <i class="fas fa-quote-left story-quote-icon"></i>
                        <p class="f-mulish">We are dedicated to providing natural and organic options made with carefully selected ingredients, free from harmful additives.</p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
